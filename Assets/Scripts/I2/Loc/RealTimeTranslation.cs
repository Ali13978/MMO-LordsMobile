	...
	public class RealTimeTranslation : MonoBehaviour
	{
		...
		public void ExampleMultiTranslations_Blocking()
		{
			...
			if (GoogleTranslation.ForceTranslate(dictionary))
			{
				...
				TranslationQuery translationQuery = dictionary["Hola"];
				UnityEngine.Debug.Log(translationQuery.Results[0]);
			}
		}
		...
		private void OnMultitranslationReady(Dictionary<string, TranslationQuery> dict, string errorMsg)
		{
			...
			TranslatedText = TranslatedText + GoogleTranslation.GetQueryResult("This is an example", string.Empty, dict) + "\n";
			string translatedText = TranslatedText;
			TranslationQuery translationQuery = dict["Hola"];
			TranslatedText = translatedText + translationQuery.Results[0];
		}

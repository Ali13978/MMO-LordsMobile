	...
	public class JsonMapper
	{
		...
		private static void AddObjectMetadata(Type type)
		{
			...
			foreach (PropertyInfo propertyInfo in properties)
			{
				...
				string key = value2.Info.Name;
				Attribute[] customAttributes = Attribute.GetCustomAttributes(value2.Info);
				...
				value.Properties.Add(key, value2);
			}
			...
			foreach (FieldInfo fieldInfo in fields)
			{
				...
				string key2 = value3.Info.Name;
				Attribute[] customAttributes2 = Attribute.GetCustomAttributes(value3.Info);
				Attribute[] array2 = customAttributes2;
				foreach (Attribute attribute2 in array2)
				{
					JsonPropertyAttribute jsonPropertyAttribute2 = attribute2 as JsonPropertyAttribute;
					if (jsonPropertyAttribute2 != null)
					{
						key2 = jsonPropertyAttribute2.PropertyName;
						break;
					}
				}
				value.Properties.Add(key2, value3);
			}
		...
		private static object ReadValue(Type inst_type, JsonReader reader)
		{
			...
			{
				if (text != null && (text.Equals("created") || text.Equals("updated")))
				{
					return new DateTime(1970, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc).AddMilliseconds((long)reader.Value).ToLocalTime();
				}
				...
				if (inst_type.IsEnum)
				{
					return Enum.Parse(inst_type, (string)reader.Value, ignoreCase: true);
				}
				if (inst_type == typeof(DateTime) || inst_type == typeof(DateTime?))
				{
					return new DateTime(1970, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc).AddMilliseconds((long)reader.Value).ToLocalTime();
				}
			...
			else if (reader.Token == JsonToken.ObjectStart)
			{
				...
				if (inst_type.Name.Equals("BackendlessUser"))
				{
					while (true)
					{
						reader.Read();
						...
						((BackendlessUser)obj).SetProperty(key, ReadValue(obj3.GetType(), reader));
					}
				}
				else
				{
					while (true)
					{
						...
						else if (!objectMetadata.IsDictionary)
						{
							...
							{
								throw new JsonException($"The type {inst_type} doesn't have the property '{text2}'");
							}
							ReadSkip(reader);
						}
						else
						{
							((IDictionary)obj).Add(text2, ReadValue(objectMetadata.ElementType, reader));
						}
					}
				}
			}
		...
		private static void RegisterBaseExporters()
		{
			...
			{
				writer.Write((long)((DateTime)obj - new DateTime(1970, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc).ToLocalTime()).TotalMilliseconds);
			};
		...
		private static void RegisterBaseImporters()
		{
			...
			importer = ((object input) => new DateTime(1970, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc).AddMilliseconds((long)input).ToLocalTime());
		...
		private static void WriteValue(object obj, JsonWriter writer, bool writer_is_private, int depth)
		{
			...
			{
				PropertyMetadata current3 = item4;
				string property_name = current3.Info.Name;
				Attribute[] customAttributes = Attribute.GetCustomAttributes(current3.Info);
				Attribute[] array = customAttributes;
				foreach (Attribute attribute in array)
				{
					JsonPropertyAttribute jsonPropertyAttribute = attribute as JsonPropertyAttribute;
					if (jsonPropertyAttribute != null)
					{
						property_name = jsonPropertyAttribute.PropertyName;
						break;
					}
				}
				if (current3.IsField)
